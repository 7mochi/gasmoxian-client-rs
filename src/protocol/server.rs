/// Inbound message types received from the server (SG_ messages).
///
/// Each struct provides a `from_bytes()` method that unpacks the wire format
/// into Rust fields, handling the MinGW32/GCC bitfield layout.
...
impl EverythingKart {
    pub fn from_bytes(data: &[u8]) -> Self {
        EverythingKart {
            ...
            wumpa: (data[0] >> 4) & 0x07,
            reserves: (data[0] >> 7) != 0,
            kart_rotation1: (data[1] >> 3) & 0x1F,
            kart_rotation2: data[2],
            button_hold: data[3],
            position_x: i16::from_le_bytes([data[4], data[5]]),
            position_y: i16::from_le_bytes([data[6], data[7]]),
            position_z: i16::from_le_bytes([data[8], data[9]]),
        }
    }
}
...
impl MessageEngine {
    pub fn from_bytes(data: &[u8]) -> Self {
        MessageEngine {
            client_id: (data[0] >> 4) & 0x07,
            engine_type: data[1] & 0x0F,
            locked_in: (data[1] >> 4) & 1 != 0,
        }
    }
}
